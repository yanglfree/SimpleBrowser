...
| --- | --- | --- |
| Core browsing | complete | Native home, tabs, private tabs, live WebView budget, omni bar, custom `%s` search templates with prefix overrides, reader, find, desktop UA, sharing, bookmarks, history, downloads, blocking, and site permissions exist. |
...
| Start page | partial | Persistent quick-site add/edit/remove/reorder, visibility/count, built-in backgrounds, a private downsampled custom-photo cache, and an opt-in Bing daily background now exist. Daily refresh uses today's cache, then a validated HTTPS fetch, then stale cache and bundled fallback. PhotosPicker import, daily refresh/failure, rotation cropping, and relaunch persistence still need Simulator/device acceptance. |
...
| Navigation/session | partial | Back/forward navigation, history picker, pinned tabs, adjacent close selection, configurable recent-WebView budget, expiry archive/recovery, soft-limit cleanup, non-private session restore, recent-closed recovery, cyclic tab switching, common keyboard commands, group-safe tab reordering, 30-day scroll/reader-position resume, and sanitized non-password form restoration exist. Tab-card swipe now obeys persisted master/tab-switch gesture controls and retains accessibility actions; page scrolling can collapse and reveal the toolbar. Harmony action/blocking gestures are still missing, while card swipe, toolbar motion, and drag/drop need physical-device touch acceptance. |
