...
| Blocking controls | partial | Global and per-site network/tracker/cosmetic controls, standard/strict cleanup, automatic reader policy, coalesced bundled-rule reload status, a site panel, and honest WebKit-observable cleanup categories/statistics exist. Remote subscription updates and request-level network counts remain. |
